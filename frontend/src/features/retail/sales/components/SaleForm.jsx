...
const SaleForm = ({
  ...
  const productMap = useMemo(
    () =>
      new Map(
        products.map((product) => [String(product.barcode || "").trim(), product]),
      ),
    [products],
  );
  ...
  return (
    <Box component="form" onSubmit={handleSubmit} sx={{ display: "grid", gap: 2 }}>
      {submitError ? <Alert severity="error" sx={{ borderRadius: 1.5 }}>{submitError}</Alert> : null}
      <AppTextField
        ...
        {formData.items.map((item, index) => {
          ...
          return (
            <Paper key={index} elevation={0} sx={{ p: 1.5, border: "1px solid #F9D5DC", borderRadius: 2 }}>
              <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "1.3fr 1.6fr 1fr 0.8fr auto" }, gap: 1.25, alignItems: "center" }}>
                <AppTextField
                  label="Barcode"
                  value={item.barcode}
                ...
                />
                <AppTextField label="Nama Produk" value={matchedProduct?.name || ""} disabled />
                <AppTextField label="Harga Produk" value={matchedProduct ? money(matchedProduct.selling_price) : ""} disabled />
